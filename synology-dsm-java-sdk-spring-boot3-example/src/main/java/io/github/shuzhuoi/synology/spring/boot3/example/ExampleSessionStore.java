...
public class ExampleSessionStore implements SynologySessionStore {
    ...
    @Override
    public SynologyCachedSession get(SynologySessionKey key) { return sessions.get(key); }
    @Override
    public void put(SynologySessionKey key, SynologyCachedSession session) { sessions.put(key, session); }
    @Override
    public void remove(SynologySessionKey key) { sessions.remove(key); }
    @Override
