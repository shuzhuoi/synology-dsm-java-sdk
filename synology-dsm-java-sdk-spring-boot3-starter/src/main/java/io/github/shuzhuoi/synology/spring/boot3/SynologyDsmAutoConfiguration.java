...
/**
 * Spring Boot 3 自动装配。该类只负责把配置和扩展点组装成 core 客户端。
 */
@AutoConfiguration
...
public class SynologyDsmAutoConfiguration {
    ...
    public SynologyDsmClient synologyDsmClient(
        ...
        return SynologyDsmClient.builder().config(config).httpClient(httpClient)
                .jsonCodec(jsonCodec)
                .sessionStore(sessionStoreProvider.getIfAvailable()).build();
    }
}
